...
This repository contains the **RESTful Microservices Implementation** of the **Behavior-Bound Digital Twin (BBDT)** protocol, as presented in our research for **IEEE Access**.

The transition to Sixth-Generation (6G) Vehicle-to-Everything (V2X) networks demands security architectures that go beyond static cryptographic credentials, which are vulnerable to physical tampering and replay attacks. This research introduces a **Digital Twin Security Broker (DTSB)** hosted at the network edge. Unlike traditional PKI, the DTSB establishes a "behavior-bound identity" by enforcing a closed-loop verification mechanism:

1.  **Hardware Verification:** Validates the vehicle's physical integrity using **Inertial Measurement Unit (IMU)-based Physical Unclonable Functions (PUFs)**.
2.  **Behavioral Verification:** Validates the vehicle's intent by comparing its actual telemetry ($S_{act}$) against a predictive trajectory model ($S_{pred}$) generated by the Digital Twin.

This framework shifts V2X security from **Static Trust** to **Continuous, Physics-Aware Verification**
...
The project implements a three-tier architecture orchestrated via Laravel Middleware and Controllers:

### Tier 1: The Physical Layer (Vehicle OBU)

-   **Role:** Generates unique hardware signatures ($R_{puf}$) using IMU sensors (accelerometers and gyroscopes).
-   **Implementation:** Simulated via REST Client sending telemetry payloads.

### Tier 2: The Edge Layer (DTSB - _This Repository_)

-   **Role:** Acts as the Security Broker hosted on Multi-access Edge Computing (MEC) nodes.
-   **Key Components:**
    -   `DtsbAuthController`: Handles the 3-phase handshake (Registration, Challenge, Verification).
    -   `DigitalTwinPredictor`: Service class running the Kalman-like trajectory prediction logic ($S_{pred}$).
    -   `BehaviorBoundMiddleware`: Intercepts V2X traffic to enforce Zero Trust principles.

### Tier 3: The Trust Anchor

-   **Role:** Stores immutable logs and trust scores, replacing traditional Certificate Revocation Lists (CRLs).
-   **Implementation:** Database Layer (MySQL for logs, Redis for high-speed state caching). -->
